import { loginWithGoogle, loginWithMicrosoft, loginWithYahoo, hasUsername } from '@/services/auth'

export default async function Events() {
    try {
        setupProviders()
    } catch (error) {
        console.error('Registration Event:', error)
    }
}
...
function setupProviders() {
    const providers = [
        ...
        { id: 'microsoft-btn', login: loginWithMicrosoft },
        { id: 'yahoo-btn', login: loginWithYahoo },
    ]

    providers.forEach(({ id, login }) => {
        document.querySelector(`#${id}`)?.addEventListener('click', () =>
            handleProviderLogin(id, login),
        )
    })
}
